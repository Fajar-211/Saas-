...
const props = defineProps(['seetingbutton']);
const handle = ()=>{
    if(props.seetingbutton == false){
        nav.layout = 'setting';
        emit('open', true);
    }else{
        emit('open', false);
...
<template>
    <button type="button" @click="handle" :class="['p-2 text-sm md:text-lg font-medium text-left hover:ring-2 hover:ring-sky-200 rounded-lg flex justify-between', nav.layout == 'setting' ? 'bg-indigo-100' : 'bg-white']">
        <div class="flex gap-x-2 group">
            <slot />
            <div :class="['w-5 group-active:scale-95']">
                <svg :style="{transform: props.seetingbutton ? 'rotate(180deg)' : 'rotate(0deg)', transition: 'transform 0.5s ease'}" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
        ...
        <div class="flex inset-0">
            <svg
                :class="['m-auto h-4 w-4 transition-all duration-200', props.seetingbutton ? 'rotate-180' : 'rotate-0']"
